...
ERROR_SCREENSHOT_FILE = "error_screenshot.png" # 定义错误截图文件名
...
async def export_and_process_data(page: Page):
    ...
    try:
        ...
        async with page.context.expect_page() as new_page_info:
            await page.get_by_role("button", name="导出导出").click()
        ...
        # 如果此流程内部出错，打印错误并返回False，主函数将据此决定是否截图
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            print(f"❌ 致命错误：找不到 Cookie 文件 '{COOKIE_FILE}'。请确保该文件已上传到GitHub仓库中。")
            exit(1)
        except Exception as e:
        ...
        browser = await p.chromium.launch(headless=True) # 在服务器上必须为 True
        ...
        try:
            ...
            exit(1) # 以失败状态退出，触发GitHub Actions的failure步骤
        finally:
            print("操作完成，关闭浏览器...")
            await context.close()
            await browser.close()
