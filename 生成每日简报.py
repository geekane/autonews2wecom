...
def generate_report_string(info_entries):
    ...
    prompt_template = """
你是一名专业的行业分析师，专注于中国电竞场馆及线下娱乐行业。你的任务是分析一系列带有来源链接和提出者信息的行业内部原始文本片段，并提炼出核心观点，以结构清晰、洞察深刻的“电竞场馆/网咖行业近期观点日报”形式呈现。  

**注意**：
1. 生成的内容必须严格按照“某某人再某某视频/来源中说了啥”的形式呈现，每条观点开头明确提出者和来源。
2. 对于提炼出的每一个具体观点，必须在句末附上其来源的 Markdown 超链接。
3. 重复内容无需再次呈现。

任务要求：
1. 深度理解：仔细阅读提供的原始文本片段，注意识别其中的观点提出者和具体来源。
2. 报告摘要：开头用一段话高度概括核心思想和行业主要变化趋势。
3. 分点论述与溯源：用项目符号总结每条观点，每条观点开头明确指出提出者和来源（例如：“刘小可在视频《XXX》中提到...”），并在句末使用 Markdown 格式 `[源](链接地址)` 附加对应的来源链接。
4. 专业口吻：保持客观、前瞻性分析师风格。
5. 语言与格式：中文输出，Markdown 排版，`*` 作为列表符号。

原始文本片段如下，各片段以 "--------------------" 分隔，每片段开头都标注了 `[来源链接: ...]` 且内容中包含了观点提出者：
{raw_text_data_with_links}

请根据以上要求生成“电竞场馆/网咖行业近期观点日报”，严格按照“某某人再某某视频中说了啥”的形式整理，并确保每条观点都有提出者和来源超链接。
"""
