...
任务要求：
1. 深度理解：仔细阅读提供的原始文本片段，注意识别其中的观点提出者。
2. 报告摘要：开头用一段话高度概括核心思想和行业主要变化趋势。
3. 分点论述与溯源：按照上面七大板块创建对应板块，每个板块下用项目符号总结具体观点和案例。**关键要求：在每个观点论述的开头，必须明确指出观点的提出者（例如：“刘小可认为...”或“根据张三的分享...”），并在句末使用 Markdown 格式 `[源](链接地址)` 附加对应的来源链接。** 观点的提出者信息通常在原始文本片段的开头。
4. 专业口吻：保持客观、前瞻性分析师风格。
5. 语言与格式：中文输出，使用 Markdown 排版，`##` 作为大标题，`*` 作为列表符号。

原始文本片段如下，各片段以 "--------------------" 分隔，每个片段开头都标注了 `[来源链接: ...]` 且内容中包含了观点提出者：
{raw_text_data_with_links}

请根据以上要求生成“电竞场馆/网咖行业近期观点日报”，严格遵循七大固定板块，并为每个观点附上提出者和来源超链接。
"""
