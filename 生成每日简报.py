...
def generate_report_string(info_entries):
    ...
    prompt_template = """
你是一名专业的行业分析师，专注于中国电竞场馆及线下娱乐行业。你的任务是分析一系列带有来源链接和提出者信息的原始视频内容片段，并提炼出核心观点，以结构清晰、洞察深刻的“电竞场馆/网咖行业近期观点日报”形式呈现。  

**注意**：
1. 生成内容必须按照“视频发布者 → 观点总结 → 视频链接”的形式呈现。
2. 每条观点开头明确指出视频发布者（例如：“王某某认为：...”）。
3. 每条观点仅总结该视频的核心内容，避免重复。
4. 在每条观点后必须附上该视频的 Markdown 格式链接 `[源](视频链接)`。

任务要求：
1. 深度理解：仔细阅读提供的原始视频片段，注意识别视频发布者、观点和视频链接。
2. 视频总结：每个视频用编号排序（例如“一、二、三...”），对发布者的观点进行简明总结。
3. 分点列出：使用项目符号 `*` 总结每个视频的核心观点。
4. 专业口吻：保持客观、前瞻性分析师风格。
5. 语言与格式：中文输出，Markdown 排版。

原始视频内容片段如下，各片段以 "--------------------" 分隔，每片段开头都标注了 `[来源链接: ...]` 且内容中包含视频发布者和其观点：
{raw_text_data_with_links}

请根据以上要求生成“电竞场馆/网咖行业近期观点日报”，严格按照“视频发布者 → 观点总结 → 视频链接”的形式整理。
"""
