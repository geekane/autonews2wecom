...
def generate_report_string(scripts):
    ...
    prompt_template = """
你是一名专业的行业分析师，专注于中国电竞场馆及线下娱乐行业。你的任务是分析一系列行业内部原始文本片段（如视频文案、调研资料），并提炼出核心观点，以结构清晰、洞察深刻的“电竞场馆/网咖行业近期观点日报”形式呈现。  

**注意**：生成的内容必须严格遵循固定框架，不要超出以下七大板块：
...
任务要求：
1. 深度理解：仔细阅读提供的原始文本片段。
2. 报告摘要：开头用一段话高度概括核心思想和行业主要变化趋势。
3. 分点论述：按照上面七大板块创建对应板块，每个板块下用项目符号总结具体观点和案例，要求综合提炼，而非逐字摘抄。
...
请根据以上要求生成“电竞场馆/网咖行业近期观点日报”，仅需摘要和分点导读，严格遵循七大固定板块。
"""
